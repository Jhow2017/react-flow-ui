...
const globalButtonStyles = (attr: DsButtonType): object => ({
    borderRadius: (attr.size === 'small' && '6px') || ((attr.size === 'medium' || attr.size === 'large') && '10px'),
    fontSize: ((attr.size === 'small' || attr.size === 'medium') && '14px') || (attr.size === 'large' && '18px'),
    lineHeight: ((attr.size === 'small' || attr.size === 'medium') && '16px') || (attr.size === 'large' && '18px'),
    fontWeight: '600',
    ...
    height:
        (attr.size === 'small' && '36px') || (attr.size === 'medium' && '48px') || (attr.size === 'large' && '65px'),
});
...
const primaryButtonStyles = (attr: DsButtonType, dark?: boolean): object => ({
    ...
    _hover: { bg: dark ? 'var(--writer)' : attr.disabled ? 'var(--inactive)' : 'var(--hoverBlue)', ...attr._hover },
});

const secondaryButtonStyles = (attr: DsButtonType, dark?: boolean, grey?: boolean): object => ({
    _styles: {
        p:
            (attr.size === 'small' && '8px 14px') ||
            (attr.size === 'medium' && '14px 22px') ||
            (attr.size === 'large' && '21.5px 19.5px'),
            ...
            ? `2px solid ${
                  attr.disabled
                      ? dark
                          ? 'var(--tagGrey)'
                          : 'var(--inactive)'
                      : (dark && 'var(--decorGrey)') || (grey && 'var(--writer)') || 'var(--hoverBlue)'
              }`
            : 'none',
        color: attr.disabled
            ? dark
...
const tertiaryButtonStyles = (attr: DsButtonType, dark?: boolean): object => ({
    _styles: {
        p:
            ...
            (attr.size === 'large' && (attr.disabled ? '21.5px 19.5px' : '23.5px 21.5px')),
        bg: attr.disabled
            ? dark
                ? 'var(--additionalBlack)'
                : 'var(--white) !important'
            : 'var(--intenseGradient) !important',
        border: attr.disabled ? `2px solid ${dark ? 'var(--headline)' : 'var(--inactive)'}` : 'none',
        position: 'relative',
        zIndex: '1',
        overflow: 'hidden',
        color: attr.disabled ? (dark ? 'var(--platinum)' : 'var(--inactive)') : dark ? 'var(--white)' : 'var(--writer)',
        ...globalButtonStyles(attr),
        ...iconStyles(attr.icon, attr.iconAfter),
        ...attr,
    },
...
const DsButton: React.FC<DsButtonType> = forwardRef((props, ref) => {
    ...
    if (attr.variant === 'primary') {
        styles = primaryButtonStyles(attr);
    } else if (attr.variant === 'primary-dark') {
        styles = primaryButtonStyles(attr, true);
    } else if (attr.variant === 'secondary') {
        styles = secondaryButtonStyles(attr);
    } else if (attr.variant === 'secondary-dark') {
        styles = secondaryButtonStyles(attr, true);
    } else if (attr.variant === 'secondary-grey') {
        styles = secondaryButtonStyles(attr, false, true);
    } else if (attr.variant === 'tertiary') {
        styles = tertiaryButtonStyles(attr);
    } else if (attr.variant === 'tertiary-dark') {
        styles = tertiaryButtonStyles(attr, true);
    } else if (attr.variant === 'quaternary') {
        styles = quaternaryButtonStyles(attr);
    } else if (attr.variant === 'quinary') {
        styles = quinaryButtonStyles(attr);
    }
    ...
    const button = (
        ...
        >
            ...
            <Loading className={attr.loading ? 'active' : ''}>{attr.loading && <Dots />}</Loading>
            {attr.icon && attr.icon}
            {children}
        </ComponentMounter>
    );
